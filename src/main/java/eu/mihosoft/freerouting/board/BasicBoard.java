...
/**
 * Provides basic functionality of a board with geometric items. Contains functions such as inserting, deleting,
 * modifying and picking items and elementary checking functions. A board may have 1 or several layers.
 *
 * @author Alfons Wirtz
 */
public class BasicBoard implements java.io.Serializable {
    ...
    /**
     * Creates a new instance of a routing Board with surrounding box p_bounding_box Rules contains the restrictions to
     * obey when inserting items. Among other things it may contain a clearance matrix. p_observers is used for
     * synchronisation, if the board is generated by a host database. Otherwise it is null. If p_test_level  !=
     * RELEASE_VERSION,, some features may be used, which are still in experimental state. Also warnings  for debugging
     * may be printed depending on the size of p_test_level.
     */
     ...
     * clearance_matrix, which describes the required clearance restrictions to other items. Because no internal
     * cleaning of items is done, the new inserted item can be returned.
     */
    ...
    /**
     * Inserts a trace into the board, whose geometry is described by a Polyline. p_clearance_class is the index in the
     * clearance_matrix, which describes the required clearance restrictions to other items.
     */
    ...
    /**
     * Inserts a via into the board. p_attach_allowed indicates, if the via may overlap with smd pins of the same net.
     */
    ...
    /**
     * Inserts a pin into the board. p_pin_no is the number of this pin in the library package of its component
     * (starting with 0).
     */
    ...
    /**
     * Inserts an obstacle into the board , whose geometry is described by a polygonal shape, which may have holes. If
    ...
    /**
     * Inserts an obstacle belonging to a component into the board p_name is to identify the corresponding ObstacleArea
    ...
    /**
     * Inserts an via obstacle area into the board , whose geometry is described by a polygonal shape, which may have
     * holes.
     */
    ...
    /**
     * Inserts an via obstacle belonging to a component into the board p_name is to identify the corresponding
     * ObstacleArea in the component package.
     */
    ...
    /**
     * Inserts a component obstacle area into the board , whose geometry is described by a polygonal shape, which may
     * have holes.
     */
    ...
    /**
     * Inserts a component obstacle belonging to a component into the board. p_name is to identify the corresponding
     * ObstacleArea in the component package.
     */
    ...
    /**
     * Inserts a conduction area into the board , whose geometry is described by a polygonal shape, which may have
     * holes. If p_is_obstacle is false, it is possible to route through the conduction area with traces and vias of
     * foreign nets.
     */
    ...
    /**
     * Removes the items in p_item_list. Returns false, if some items could not be removed, bcause they are fixed.
     */
    ...
    /**
     * Combines the connected traces of this net, which have only 1 contact at the connection point. if p_net_no
     * {@literal <} 0 traces of all nets are combined.
     */
     ...
     * these traces. Returns false, if no trace was split.
     */
    ...
    /**
     * Returns all SearchTreeObjects on layer p_layer, which overlap with p_shape. If p_layer {@literal <} 0, the layer
     * is ignored
     */
    ...
    /**
     * Returns items, which overlap with p_shape on layer p_layer inclusive clearance. p_clearance_class is the index in
     * the clearance matrix, which describes the required clearance restrictions to other items. The function may also
     * return items, which are nearly overlapping, but do not overlap with exact calculation. If p_layer {@literal <} 0,
    ...
    /**
     * Returns all items on layer p_layer, which overlap with p_area. If p_layer {@literal <} 0, the layer is ignored
     */
    ...
    /**
     * Checks, if the an object with shape p_shape and net nos p_net_no_arr and clearance class p_cl_class can be
     * inserted on layer p_layer without clearance violation.
     */
    ...
    /**
     * Checks, if the a trace line with shape p_shape and net numbers p_net_no_arr and clearance class p_cl_class can be
     * inserted on layer p_layer without clearance violation. If p_contact_pins != null, all pins not contained in
    ...
    /**
     * Draws all items of the board on their visible layers. Called in the overwritten paintComponent method of a class
     * derived from JPanel. The value of p_layer_visibility is expected between 0 and 1 for each layer.
     */
    ...
    /**
     * Returns the list of items on the board, whose shape on layer p_layer contains the point at p_location. If p_layer
     * {@literal <} 0, the layer is ignored. If p_item_selection_filter != null, only items of types selected by the
     * filter are picked.
     */
    ...
    /**
     * Returns the minimum clearance requested between items of clearance class p_class_1 and p_class_2. p_class_1 and
     * p_class_2 are indices in the clearance matrix.
     */
    ...
    /**
     * Turns an obstacle area into a conduction area with net number p_net_no If it is convex and has no holes, it is
     * turned into a Pin, alse into a conduction area.
     */
    ...
    /**
     * Restores the sitiation at the previous snapshot. Returns false, if no more undo is possible. Puts the numbers of
     * the changed nets into the set p_changed_nets, if p_changed_nets != null
     */
    public boolean undo(Set<Integer> p_changed_nets) {
        ...
        while (it.hasNext()) {
            ...
            searchTreeManager.remove(curr_item);
        ...
        while (it.hasNext()) {
            ...
            searchTreeManager.insert(curr_item);
    ...
    /**
     * Restores the sitiation before the last undo. Returns false, if no more redo is possible. Puts the numbers of the
     * changed nets into the set p_changed_nets, if p_changed_nets != null
     */
    ...
    /**
     * Removes the top snapshot from the undo stack, so that its situation cannot be restored any more. Returns false,
     * if no more snapshot could be popped.
     */
    ...
    /**
     * Looks if at the input position ends a trace with the input net number, which has no normal contact at that
     * position. Returns null, if no tail is found.
     */
    ...
    /**
     * Checks, if p_item item is part of a cycle and remuve it together with its connection in this case.
     */
    ...
    /**
     * If != RELEASE_VERSION,, some features may be used, which are still in experimental state. Also warnings for
                   ...
                   java.lang.ClassNotFoundException
    {
        ...
        searchTreeManager = new SearchTreeManager(this);
        for (final Item currItem : get_items()) {
            currItem.board = this;
    ...
    /**
     * If test_level != RELEASE_VERSION, some features may be used, which are still in experimental state. Also warnings
     * for debugging may be printed depending on the size of test_level.
     */
