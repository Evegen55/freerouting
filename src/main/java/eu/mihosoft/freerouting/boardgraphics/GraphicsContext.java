...
public class GraphicsContext implements java.io.Serializable {
    public GraphicsContext(IntBox p_design_bounds,
                           Dimension p_panel_bounds, eu.mihosoft.freerouting.board.LayerStructure p_layer_structure, java.util.Locale p_locale) {
    ...
    /**
     * Changes  the bounds of the board design to p_design_bounds. Useful when components are still placed outside the
     * boaed.
     */
    public void change_design_bounds(IntBox p_new_design_bounds) {
        if (p_new_design_bounds.equals(this.coordinateTransform.design_box)) {
            return;
        }
        Dimension screen_bounds = this.coordinateTransform.screen_bounds;
        this.coordinateTransform = new CoordinateTransform(p_new_design_bounds, screen_bounds);
    }
    ...
    public void change_panel_size(Dimension p_new_bounds) {
        ...
        IntBox design_box = coordinateTransform.design_box;
        boolean left_right_swapped = coordinateTransform.is_mirror_left_right();
        boolean top_bottom_swapped = coordinateTransform.is_mirror_top_bottom();
        double rotation = coordinateTransform.get_rotation();
        coordinateTransform = new CoordinateTransform(design_box, p_new_bounds);
        coordinateTransform.set_mirror_left_right(left_right_swapped);
        coordinateTransform.set_mirror_top_bottom(top_bottom_swapped);
    ...
    public void draw(FloatPoint[] p_points, double p_half_width, Color p_color, Graphics p_g, double p_translucency_factor) {
        ...
        IntBox clip_box = coordinateTransform.screen_to_board(clip_shape);
        double scaled_width = coordinateTransform.board_to_screen(p_half_width);
    ...
    public void draw_circle(FloatPoint p_center, double p_radius, double p_draw_half_width,
        ...
        float draw_width = (float) (2 * coordinateTransform.board_to_screen(p_draw_half_width));
        Ellipse2D circle =
    ...
    /**
     * Draws the interior of an array  of ellipses. Ellipses contained in an other ellipse are treated as holes.
     */
    public void fill_ellipse_arr(Ellipse[] p_ellipse_arr, Graphics p_g, Color p_color, double p_translucency_factor) {
        ...
        for (Ellipse curr_ellipse : p_ellipse_arr) {
            ...
            double bigger_radius = coordinateTransform.board_to_screen(curr_ellipse.bigger_radius);
            if (!point_near_rectangle(center.getX(), center.getY(), p_g.getClip().getBounds(), bigger_radius)) {
                continue;
            }
            double smaller_radius = coordinateTransform.board_to_screen(curr_ellipse.smaller_radius);
    ...
    public void fill_shape(FloatPoint[] p_points, Graphics p_g, Color p_color, double p_translucency_factor) {
        ...
        for (int i = 0; i < p_points.length; i++) {
            Point2D curr_corner = coordinateTransform.board_to_screen(p_points[i]);
    ...
    /**
     * Fill the interior of a list of polygons. Used for example with an area consisting of a border polygon and some
     * holes.
     */
    public void fill_area(FloatPoint[][] p_point_lists, Graphics p_g, Color p_color, double p_translucency_factor) {
        ...
        for (int j = 0; j < p_point_lists.length; ++j) {
            ...
            for (int i = 0; i < curr_point_list.length; i++) {
                Point2D curr_corner = coordinateTransform.board_to_screen(curr_point_list[i]);
                draw_polygon.addPoint((int) Math.round(curr_corner.getX()),
                                      (int) Math.round(curr_corner.getY()));
            }
    ...
    public Dimension getPanelSize() {
        return coordinateTransform.screen_bounds;
    }
    ...
    public Point2D getDesignCenter() {
        FloatPoint center = coordinateTransform.design_box_with_offset.centre_of_gravity();
    ...
    public Rectangle getDesignBounds() {
        return coordinateTransform.board_to_screen(coordinateTransform.design_box);
    }

    /**
     * Sets the factor for automatic layer dimming. Values are between 0 and 1. If 1, there is no automatic layer
     * dimming.
     */
    ...
    /**
     * Gets the visibility factor of the input layer. The result is between 0 and 1. If the result is 0, the layer is
     * invisible, if the result is 1, the layer is fully visible.
     */
    ...
    /**
     * Gets the visibility factor of the input layer. The value is expected between 0 and 1. If the value is 0, the
     * layer is invisible, if the value is 1, the layer is fully visible.
     */
    ...
    /**
     * filter lines, which cannot touch the update_box to improve the performance of the draw function by avoiding
    ...
    /**
     * The factor for autoomatic layer dimming of layers different from the current layer. Values are between 0 and 1.
