...
public class WindowSnapshot extends BoardSavableSubWindow {
    ...
    public WindowSnapshot(BoardFrame p_board_frame) {
        ...
        this.resources = java.util.ResourceBundle.getBundle("eu.mihosoft.freerouting.gui.WindowSnapshot", p_board_frame.get_locale());
        ...
        p_board_frame.setContextSensitiveHelp(this, "WindowSnapshots");
        ...
        if (index >= 0 && list_model.getSize() > index) {
            ...
            if (curr_snapshot.settings.get_snapshot_attributes().display_region) {
                ...
                if (viewport_position != null) {
                    board_handling.graphicsContext.coordinateTransform = new eu.mihosoft.freerouting.boardgraphics.CoordinateTransform(curr_snapshot.graphics_context.coordinateTransform);
                    java.awt.Dimension panel_size = board_handling.graphicsContext.getPanelSize();
            ...
            board_frame.hilightSelectedButton();
    ...
    /**
     * Selects the item, which is previous to the current selected item in the list. The current selected item is then
     * no more selected.
     */
    ...
    /**
     * Selects the item, which is next to the current selected item in the list. The current selected item is then no
     * more selected.
     */
