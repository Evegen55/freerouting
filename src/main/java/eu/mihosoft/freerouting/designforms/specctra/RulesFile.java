...
public class RulesFile {
    ...
    private static void write_rules(WriteScopeParameter p_par, String p_design_name) throws java.io.IOException {
        ...
        for (int i = 1; i <= p_par.board.library.padstacks.count(); ++i) {
            eu.mihosoft.freerouting.library.Padstack curr_padstack = p_par.board.library.padstacks.get(i);
    ...
    private static boolean read_via_info(Scanner p_scanner, BasicBoard p_board) {
        eu.mihosoft.freerouting.rules.ViaInfo curr_via_info = Network.read_via_info(p_scanner, p_board);
        if (curr_via_info == null) {
            return false;
        }
        eu.mihosoft.freerouting.rules.ViaInfo existing_via = p_board.rules.via_infos.get(curr_via_info.get_name());
