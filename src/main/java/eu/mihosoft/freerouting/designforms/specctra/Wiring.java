...
class Wiring extends ScopeKeyword
{
    ...
    public static void write_scope(WriteScopeParameter p_par) throws java.io.IOException
    {
        ...
        for(;;)
        {
            ...
            write_conduction_area_scope(p_par, (eu.mihosoft.freerouting.board.ConductionArea) curr_ob);
        }
    ...
    private static void write_wire_scope(WriteScopeParameter p_par, Trace p_wire) throws java.io.IOException
    {
        ...
        eu.mihosoft.freerouting.rules.Net wire_net = null;
    ...
    static private void write_net(eu.mihosoft.freerouting.rules.Net p_net, IndentFileWriter p_file, IdentifierType p_identifier_type) throws java.io.IOException
    {
        p_file.new_line();
        p_file.write("(");
    ...
    private Item read_wire_scope(ReadScopeParameter p_par)
    {
        ...
        int curr_index = 0;
        for (eu.mihosoft.freerouting.rules.Net curr_net : found_nets)
        {
        ...
        if (border_shape != null)
        {
            ...
            {
                clearance_class_no =
                        net_class.default_item_clearance_classes.get(eu.mihosoft.freerouting.rules.DefaultItemClearanceClasses.ItemClass.AREA);
            }
        ...
        else if (path instanceof PolygonPath)
        {
            ...
            {
                clearance_class_no =
                        net_class.default_item_clearance_classes.get(eu.mihosoft.freerouting.rules.DefaultItemClearanceClasses.ItemClass.TRACE);
            }
    ...
    private static Collection<eu.mihosoft.freerouting.rules.Net> get_subnets(Net.Id p_net_id, eu.mihosoft.freerouting.rules.BoardRules p_rules)
    {
        Collection<eu.mihosoft.freerouting.rules.Net> found_nets = new LinkedList<eu.mihosoft.freerouting.rules.Net>();
        if (p_net_id != null)
        {
            if (p_net_id.subnet_number > 0)
            {
                eu.mihosoft.freerouting.rules.Net found_net = p_rules.nets.get(p_net_id.name, p_net_id.subnet_number);
                if (found_net != null)
                {
    ...
    private boolean read_via_scope(ReadScopeParameter p_par)
    {
        try
        {
            ...
            if (clearance_class_no < 0)
            {
                clearance_class_no =  net_class.default_item_clearance_classes.get(eu.mihosoft.freerouting.rules.DefaultItemClearanceClasses.ItemClass.VIA);
            }
    ...
    private static boolean via_exists(IntPoint p_location, eu.mihosoft.freerouting.library.Padstack p_padstack,
            int[] p_net_no_arr, eu.mihosoft.freerouting.board.BasicBoard p_board)
    {
    ...
    static eu.mihosoft.freerouting.board.FixedState calc_fixed(Scanner p_scanner)
    {
        try
        {
            ...
            {
                result = eu.mihosoft.freerouting.board.FixedState.SYSTEM_FIXED;
            }
