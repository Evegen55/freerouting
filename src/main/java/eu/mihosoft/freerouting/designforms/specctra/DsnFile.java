...
public class DsnFile {
    ...
    /**
     * Creates a routing board from a Specctra dns file. The parameters p_item_observers and p_item_id_no_generator are
     * used, in case the board is embedded into a host system. Returns false, if an error occured.
     */
    ...
    /**
     * Sets contains_plane to true for nets with a conduction_area covering a large part of a signal layer, if that
     * layer does not contain any traces This is useful in case the layer type was not set correctly to plane in the
     * dsn-file. Returns true, if something was changed.
     */
    ...
    /**
     * Writes p_board to a text file in the Specctra dsn format. Returns false, if the write failed. If p_compat_mode is
     * true, only standard speecctra dsn scopes are written, so that any host system with an specctra interface can read
     * them.
     */
