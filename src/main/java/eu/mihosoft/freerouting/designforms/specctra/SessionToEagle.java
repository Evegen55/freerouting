...
public class SessionToEagle extends javax.swing.JFrame
{
    ...
    SessionToEagle(Scanner p_scanner, java.io.OutputStreamWriter p_out_file, eu.mihosoft.freerouting.board.BasicBoard p_board,
                   eu.mihosoft.freerouting.board.Unit p_unit, double p_session_file_scale_dominator, double p_board_scale_factor)
    {
        scanner = p_scanner;
        out_file = p_out_file;
        board = p_board;
    ...
    private boolean process_session_scope() throws java.io.IOException
    {
        ...
        eu.mihosoft.freerouting.geometry.planar.IntBox board_bounding_box = this.board.get_bounding_box();
    ...
    private boolean process_via_scope(String p_net_name) throws java.io.IOException
    {
        ...
        eu.mihosoft.freerouting.library.Padstack via_padstack = this.board.library.padstacks.get(padstack_name);
        ...
        eu.mihosoft.freerouting.geometry.planar.ConvexShape via_shape = via_padstack.get_shape(via_padstack.from_layer());
        ...
        if (via_shape instanceof eu.mihosoft.freerouting.geometry.planar.Circle)
        {
            this.out_file.write(" round ");
        }
        else if (via_shape instanceof eu.mihosoft.freerouting.geometry.planar.IntOctagon)
        {
    ...
    {
        java.util.Collection<eu.mihosoft.freerouting.board.Pin> component_pins = this.board.get_component_pins(p_component_no);
        boolean component_has_swapped_pins = false;
        for (eu.mihosoft.freerouting.board.Pin curr_pin : component_pins)
        {
    ...
    private void write_pin_swap(eu.mihosoft.freerouting.board.Pin p_pin_1, eu.mihosoft.freerouting.board.Pin p_pin_2) throws java.io.IOException
    {
    ...
    private final eu.mihosoft.freerouting.board.Unit unit;
    ...
    {
        PinInfo(eu.mihosoft.freerouting.board.Pin p_pin)
        {
            pin = p_pin;
            curr_changed_to = p_pin;
        }
        final eu.mihosoft.freerouting.board.Pin pin;
        eu.mihosoft.freerouting.board.Pin curr_changed_to;
    }
}
