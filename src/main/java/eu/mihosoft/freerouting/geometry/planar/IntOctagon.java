...
/**
 * Implements functionality for convex shapes, whose borderline directions are multiples of 45 degree and defined with
 * integer coordinates.
 *
 * @author Alfons Wirtz
 */
...
public class IntOctagon extends RegularTileShape implements Serializable {
    ...
    /**
     * Creates an IntOctagon from 8 integer values. p_lx is the smallest x value of the shape. p_ly is the smallest y
     * value of the shape. p_rx is the biggest x valuje af the shape. p_uy is the biggest y value of the shape. p_ulx is
     * the intersection of the upper left diagonal boundary line with the x axis. p_lrx is the intersection of the lower
     * right diagonal boundary line with the x axis. p_llx is the intersection of the lower left diagonal boundary line
     * with the x axis. p_urx is the intersection of the upper right diagonal boundary line with the x axis.
     */
    ...
    /**
     * Returns true, if p_point is contained in this octagon. Because of the parameter type FloatPoint, the function may
     * not be exact close to the border.
     */
    ...
    /**
     * Calculates the side of the point (p_x, p_y)  of the border line with index p_border_line_no. The border lines are
     * located in counterclock sense around this octagon.
     */
    ...
    /**
     * Calculates the border point of this octagon from p_point into the 45 degree direction p_dir. If this border point
     * is not an IntPoint, the nearest outside IntPoint of the octagon is returned.
     */
    ...
    /**
     * Calculates the sorted  p_max_result_points nearest points on the border of this octagon in the 45-degree
     * directions. p_point is assumed to be located in the interiour of this octagon.
     */
    public IntPoint[] nearest_border_projections(IntPoint p_point, int p_max_result_points) {
        ...
        FloatPoint inside_point = p_point.toFloat();
        for (FortyfiveDegreeDirection curr_dir : FortyfiveDegreeDirection.values()) {
            ...
            double curr_dist = inside_point.distanceSquare(curr_border_point.toFloat());
            for (int i = 0; i < p_max_result_points; ++i) {
