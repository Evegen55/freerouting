 ...
 * <p>
 * Because arithmetic calculations with double's are in general not exact,
 * <p>
...
public class FloatPoint implements Serializable {
    ...
    /**
     * returns the square of the distance from this Point to the Point floatPoint
     */
    public final double distanceSquare(FloatPoint floatPoint) {
        double dx = floatPoint.x - x;
        double dy = floatPoint.y - y;
        return dx * dx + dy * dy;
    }
    ...
    /**
     * Computes the weighted distance to floatPoint.
     */
    public double weightedDistance(
        ...
        double delta_x = this.x - floatPoint.x;
        double delta_y = this.y - floatPoint.y;
        delta_x *= horizontalWeight;
        delta_y *= verticalWeight;
    ...
    /**
     * Rounds this point, so that if this point
     * <p>
     * is on the right side of any directed line with direction direction,
     * <p>
     * the result point will also be on the right side.
     */
    public IntPoint roundToTheRight(Direction direction) {
        FloatPoint dir = direction.get_vector().toFloat();
        int rounded_x;
    ...
    /**
     * Round this Point so the x coordinate of the result
     * <p>
     * will be a multiple of horizontalGrid
     * <p>
     * and
     * <p>
     * the y coordinate a multiple of verticalGrid.
     */
    public IntPoint roundToGrid(int horizontalGrid, int verticalGrid) {
        double rounded_x;
        if (horizontalGrid > 0) {
            rounded_x = Math.rint(this.x / horizontalGrid) * horizontalGrid;
        } else {
            rounded_x = this.x;
        }
        double rounded_y;
        if (verticalGrid > 0) {
            rounded_y = Math.rint(this.y / verticalGrid) * verticalGrid;
        } else {
            rounded_y = this.y;
        }
     ...
     * Rounds this point, so that if this point
     * <p>
     * is on the left side of any directed line with direction direction,
     * <p>
     * the result point will also be on the left side.
     */
    public IntPoint roundToTheLeft(Direction direction) {
        FloatPoint dir = direction.get_vector().toFloat();
        int rounded_x;
    ...
    /**
     * Adds the coordinates of this FloatPoint and floatPoint.
     */
    ...
    /**
     * Substracts the coordinates of floatPoint from this FloatPoint.
     */
    ...
    public FloatPoint projectionApprox(Line line) {
        ...
        return floatLine.perpendicular_projection(this);
    }

    /**
     * Calculates the scalar prodct of (floatPoint_1 - this). with (floatPoint_2 - this).
     */
    public double scalarProduct(
            FloatPoint floatPoint_1,
            FloatPoint floatPoint_2
    ) {
        if (floatPoint_1 == null || floatPoint_2 == null) {
            FRLogger.warn("FloatPoint.scalar_product: parameter point is null");
            return 0;
        }
        double dx_1 = floatPoint_1.x - this.x;
        double dx_2 = floatPoint_2.x - this.x;
        double dy_1 = floatPoint_1.y - this.y;
        double dy_2 = floatPoint_2.y - this.y;
    ...
    /**
     * The function returns Side.ON_THE_LEFT, if this Point is on the left of the line from p_1 to p_2; and
     * Side.ON_THE_RIGHT, if this Point is on the right of the line from p_1 to p_2. Collinearity is not defined,
     * becouse numerical calculations ar not exact for FloatPoints.
     */
    ...
    public boolean inside_circle(FloatPoint p_1, FloatPoint p_2, FloatPoint p_3) {
        ...
        double radius_square = center.distanceSquare(p_1);
        return (this.distanceSquare(center) < radius_square - 1); // - 1 is a tolerance for numerical stability.
    }
